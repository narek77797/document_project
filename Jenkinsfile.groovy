...
pipeline {
    agent any
    environment {
        DOCKER_REGISTRY = "pto/php-deployment"
        ...
        stage('Build and Deploy Dev') {
            ...
            steps {
                script {
                    ...
                    sh 'docker build -t myapp-dev:latest'
                    // Deploy development environment (adapt as needed)
                    sh 'docker run -d --name myapp-dev -p 8080:80 myapp-dev:latest'
                }
            }
        }
//         stage('Deploy_Dev') {
